...
| ~~`ml` (architecture-wide)~~ | **RETIRE 2026-05-17 (final)** | L61 grid found AUC signal (6/16 above 0.55). Per-asset 5-axis on top-6 showed **0/6 deployment-eligible**: all CI_lo < 0, strategy is 96-99% active (degenerate constant-trading), costs dominate the marginal AUC edge. AUC and Sharpe are different metrics — the V3.6 5-axis matrix correctly filtered out what AUC alone would have approved. | **L72 (new): classification AUC > 0.55 is NOT a deployment signal. Always close the loop with strategy-return 5-axis matrix. AUC asks "can I order two random bars correctly", Sharpe asks "does PnL distribution have favourable risk-adjusted mean". A model can do the first while failing the second due to cost drag / position sizing / payoff asymmetry. Report fraction_active alongside AUC; > 90% active = approximately constant-direction bet.** |
