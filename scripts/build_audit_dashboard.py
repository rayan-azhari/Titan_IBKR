...
REGISTRY: list[AuditEntry] = [
    ...
    AuditEntry(
        ...
        deployment_status="LIVE on docker paper account (ib-gateway:4004, client_id=21) since 2026-05-16 17:10 UTC. Replaced J4 via in-place config overwrite + docker compose restart titan-portfolio. 2 existing positions adopted cleanly from IBKR (CSPX qty=27 px=798.15, IDTM qty=45 px=172.99 — no double-fill, no flip). 2026-05-17 23:49 UTC: PROMOTED to V3.7 multi-strategy node via watchdog_portfolio.py --strategies v37_live; runs alongside turtle CAT C3_peak in same TradingNode sharing PRM + Allocator. Positions REHYDRATED cleanly through the V3.7 cutover. Next monthly rebalance will trim equity leg (J5 wants ~50% of J4 exposure). Rollback path: git revert config + container restart.",
        new_lessons=["L57"],
    ...
    # ── fx_carry Wave B audit (CONDITIONAL_WATCHPOINT long-yen scope) ─────
    AuditEntry(
        audit_id="fx_carry_v37_audit_2026_05",
        name="fx_carry V1-era V3.7 re-audit (Wave B -- CONDITIONAL long-yen-carry scope)",
        strategy_class="CARRY",
        pre_reg="(no formal pre-reg -- L60+L61+L66+L65 batch audit harness)",
        result_log=".tmp/reports/wave_b_completion/summary.md",
        run_date="2026-05-16",
        universe_size=6,
        universe_kind="FX daily (AUD/JPY live + G10 panel)",
        window_start="2010-01-04",
        window_end="2026-04-02",
        n_bars=3906,
        n_wfo_folds=0,
        canonical_cell="live (sma_period=50, vol_target=0.08)",
        canonical_sharpe=0.289,
        canonical_ci_lo=-0.233,
        plateau_spread_pct=None,
        plateau_gate_passed=True,
        matrix_verdict="CONDITIONAL_WATCHPOINT (long-yen-carry scope)",
        ci_gate_verdict="L66 PASS (cash baseline), L64 relaxed CI band: CI_lo -0.23 within -0.50",
        binding_constraint="L21 PASS. Pure-price audit gives +0.29 SR on AUD/JPY. L61 panel: only AUD/JPY (+0.29) and USD/JPY (+0.34) positive; panel median -0.007. Pattern coherent (only long-yen-carry pairs work). Pure-price audit IGNORES swap premium (~+0.6 SR boost from rate differential).",
        deployment_status="CONDITIONAL_WATCHPOINT scoped to AUD/JPY at ~10% weight. Joint L65 + L67 portfolio matrix needed before live cutover.",
        new_lessons=[],
        notes="Wave B batch audit (4 of 4). L60+L61+L66+L65+L67 applied. Pattern interesting: trend filter proxies carry direction -- positive only on classic long-yen-carry pairs (AUD/JPY, USD/JPY). Carry-premium component (swap differential) not modelled in pure-price test; true economic Sharpe likely higher.",
    ),
    # ── orb Wave B audit (TENTATIVE RETIRE, data-limited) ─────────────────
    AuditEntry(
        audit_id="orb_v37_audit_2026_05",
        name="orb V1-era V3.7 re-audit (Wave B -- TENTATIVE RETIRE data-limited)",
        strategy_class="INTRADAY_BREAKOUT",
        pre_reg="(no formal pre-reg -- L60+L61+L65 batch audit harness)",
        result_log=".tmp/reports/wave_b_completion/summary.md",
        run_date="2026-05-16",
        universe_size=7,
        universe_kind="US equity M5 (UNH/AMAT/TXN/INTC/CAT/WMT/TMO)",
        window_start="2026-02-05",
        window_end="2026-03-11",
        n_bars=1872,
        n_wfo_folds=0,
        canonical_cell="simplified ORB (no per-ticker filters)",
        canonical_sharpe=-2.50,
        canonical_ci_lo=None,
        plateau_spread_pct=None,
        plateau_gate_passed=False,
        matrix_verdict="TENTATIVE RETIRE (data insufficient)",
        ci_gate_verdict="DATA INSUFFICIENT -- only 17-19 trades/ticker over 5 weeks",
        binding_constraint="Only 5 weeks of M5 data (1872 bars/ticker = ~24 trading days = 17-19 ORB trades). Simplified-no-filter panel median per-trade SR -2.50, 29% positive (INTC + WMT only). Pattern matches L58: filter machinery layered on weak/negative base.",
        deployment_status="TENTATIVE RETIRE -- re-audit when 1+ year of M5 data available (IBKR paper, Polygon, Databento). Real live includes per-ticker SMA/RSI/Gauss filter overlays not modelled.",
        new_lessons=[],
        notes="Wave B batch audit. L21 PASS on the limited window. Cannot definitively retire on 24 days of data; the verdict is L58-pattern + DATA INSUFFICIENT. Real ORB live has bracket OCO + per-ticker filters that may rescue.",
    ),
    # ── ic_equity_daily Wave B audit (CONDITIONAL candidate) ──────────────
    AuditEntry(
        audit_id="ic_equity_daily_v37_audit_2026_05",
        name="ic_equity_daily V1-era V3.7 re-audit (Wave B -- CONDITIONAL candidate)",
        strategy_class="DAILY_MEAN_REVERSION",
        pre_reg="(no formal pre-reg -- L21+L63+L62+L66 batch audit harness)",
        result_log=".tmp/reports/wave_b_completion/summary.md",
        run_date="2026-05-16",
        universe_size=7,
        universe_kind="US equity D (HWM, CB, SYK, NOC, WMT, ABNB, GL)",
        window_start="2005-01-02",
        window_end="2026-04-02",
        n_bars=6592,
        n_wfo_folds=0,
        canonical_cell="top-3 strict-OOS: HWM (+1.12), WMT (+0.95), SYK (+0.72)",
        canonical_sharpe=0.621,
        canonical_ci_lo=None,
        plateau_spread_pct=None,
        plateau_gate_passed=True,
        matrix_verdict="CONDITIONAL_WATCHPOINT candidate (pending joint L65 + L67)",
        ci_gate_verdict="L66 PASS (cash baseline); panel median strict-OOS +0.61, 100% positive",
        binding_constraint="L21 PASS via 50/50 IS-OOS + IS-only ic_sign fit. All 7 live tickers strict-OOS SR > 0. L62 V1-gap +2.56 = single-LA bug class (V1 claimed +3-4 SR with full-series ic_sign LA; strict-OOS gives +0.6 mean). Pattern DIFFERS from ic_mtf multi-TF amplification.",
        deployment_status="CONDITIONAL_WATCHPOINT: deploy top-3 (HWM, WMT, SYK) at 5-10% total portfolio weight, joint L65 + L67 needed before live.",
        new_lessons=[],
        notes="Wave B batch audit. Real signal confirmed across all 7 live tickers. V1 +3-4 SR claim was LA-inflated by ~+2.56 SR but strict +0.6 SR is genuine. CONDITIONAL_WATCHPOINT candidate; needs joint MC vs current GEM+turtle before live cutover. Strongest Wave B verdict alongside fx_carry long-yen.",
    ),
    # ── pairs Wave B audit (RETIRE -- L21 fail + weak SR) ─────────────────
    AuditEntry(
        audit_id="pairs_v37_audit_2026_05",
        name="pairs (GLD/EFA) V1-era V3.7 re-audit (Wave B -- RETIRE L21 fail)",
        strategy_class="PAIRS",
        pre_reg="(no formal pre-reg -- L21+L66+L65 batch audit harness)",
        result_log=".tmp/reports/wave_b_completion/summary.md",
        run_date="2026-05-16",
        universe_size=2,
        universe_kind="GLD/EFA daily cointegration",
        window_start="2004-11-18",
        window_end="2026-04-02",
        n_bars=5376,
        n_wfo_folds=0,
        canonical_cell="live (entry_z=2.0, exit_z=0.5)",
        canonical_sharpe=0.111,
        canonical_ci_lo=-0.287,
        plateau_spread_pct=None,
        plateau_gate_passed=False,
        matrix_verdict="RETIRE (L21 smoke FAIL + weak SR + deep DD)",
        ci_gate_verdict="L21 PASS gate NON-NEGOTIABLE; live SR +0.11 with CI_lo -0.29 fails CONDITIONAL gate anyway",
        binding_constraint="L21 smoke FAILED (max past-return diff = 0.025). Subtle non-causality in beta-refit / z-score-window interaction. Live SR +0.11, MaxDD -30%. L62 V1-gap ~+1.0 = methodology drift class but L21 fail trumps. Would need (1) refactor for strict causality + (2) re-audit + (3) significantly higher SR to pass CONDITIONAL.",
        deployment_status="RETIRED. Not deployed in champion_portfolio. No allocator action needed.",
        new_lessons=[],
        notes="Wave B batch audit. L21 PASS is non-negotiable per V3.7. V1 claimed +1.14 OOS Sharpe with OOS/IS ratio 3.50 (suspicious overfit). Pair-trading strategies need careful beta-refit causality handling; my V3.7 implementation has a subtle bug that needs investigation if pairs is ever revisited.",
    ),
    ...
    AuditEntry(
        ...
        deployment_status="LIVE ON PAPER since 2026-05-17 23:49 UTC (multi-strategy node alongside GEM J5; Docker container titan-portfolio running watchdog_portfolio.py --strategies v37_live). C3_peak (entry=45, exit=20) on CAT at 20% weight. Joint L65 PASSED (P_kill=0.30%). Re-audit 2026-11-16 (6mo).",
