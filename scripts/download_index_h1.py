"""download_index_h1.py — Download H1 bars for foreign indices from IBKR.

IBKR caps H1 index data at 1 year per request, so this script
fetches in 1-year chunks stepping back from today.

Usage:
  uv run python scripts/download_index_h1.py --symbol FTSE --years 10
  uv run python scripts/download_index_h1.py --symbol GDAXI --years 10
"""
...
IBKR_CLIENT_ID = 13  # Must not conflict with ORB(20), MTF(11), download(10), fx_m5(12)
...
# IBKR index contract specs: symbol -> (exchange, currency)
...
def build_index_contract(symbol: str, exchange: str, currency: str) -> Contract:
    c = Contract()
    c.symbol = symbol
    c.secType = "IND"
    c.exchange = exchange
...
def main() -> None:
    ...
    parser.add_argument("--symbol", required=True, help="Index symbol: FTSE, GDAXI, SPX, INDU")
    parser.add_argument(
        "--years", type=int, default=10, help="Years of history to fetch (default: 10)"
    )
    args = parser.parse_args()

    symbol = args.symbol.upper()
    if symbol not in INDEX_SPECS:
        print(f"ERROR: Unknown symbol '{symbol}'. Add it to INDEX_SPECS in this script.")
        print(f"  Known: {list(INDEX_SPECS.keys())}")
        sys.exit(1)

    exchange, currency = INDEX_SPECS[symbol]
    output_path = DATA_DIR / f"^{symbol}_H1.parquet"
    print(f"Downloading ^{symbol} H1 ({args.years} x 1-year chunks) from IBKR...")
    ...
    contract = build_index_contract(symbol, exchange, currency)
    all_rows: list = []
    now_utc = datetime.now(timezone.utc)

    for chunk in range(args.years):
        end_ts = now_utc - timedelta(days=CHUNK_STEP_DAYS * chunk)
        end_dt = "" if chunk == 0 else end_ts.strftime("%Y%m%d-%H:%M:%S")
        rows = fetch_chunk(app, req_id=chunk + 1, contract=contract, end_dt=end_dt)
