...
def _tf_signal(close, fast_ma, slow_ma, rsi_period):
    ...
    return pd.Series(np.where(fast > slow, 0.5, -0.5), index=close.index) + pd.Series(
        np.where(r > 50, 0.5, -0.5), index=close.index
    )
...
def drawdown_periods(pf, top_n: int = 5) -> pd.DataFrame:
    ...
    return rec[
        ["Start Timestamp", "Valley Timestamp", "End Timestamp", "DD_pct", "Duration"]
...
def flat_size_sharpe(close, long_e, short_e, long_x, short_x) -> float:
    ...
    pf = run_portfolio(close, long_e, short_e, long_x, short_x, size=10_000, size_type="value")
    try:
...
def main():
    ...
    _is_end_ts = feats_act.index[split_idx - 1]
    ...
    long_e_raw = primary == 1
    long_x_raw = primary != 1
    short_e_raw = primary == -1
    short_x_raw = primary != -1
    long_e_meta = (primary == 1) & gate
    long_x_meta = long_x_raw
    short_e_meta = (primary == -1) & gate
    short_x_meta = short_x_raw
    ...
    pf_raw = run_portfolio(
        close_oos,
        long_e_raw[oos_mask],
        short_e_raw[oos_mask],
        long_x_raw[oos_mask],
        short_x_raw[oos_mask],
    )
    pf_meta = run_portfolio(
        close_oos,
        long_e_meta[oos_mask],
        short_e_meta[oos_mask],
        long_x_meta[oos_mask],
        short_x_meta[oos_mask],
    )
    ...
    print(
        f"\n  Compounding (all-cash):    Sharpe {pf_raw.sharpe_ratio():.3f}"
        f"  |  Return {pf_raw.total_return() * 100:+.1f}%"
    )
    print(
        f"  Flat-size ($10k/trade):    Sharpe {sharpe_flat_raw:.3f}"
        f"  |  Return {total_ret_flat:+.1f}%"
    )
    if pf_flat.total_return() > 0:
        ret_ratio = pf_raw.total_return() / pf_flat.total_return()
        sharpe_diff = pf_raw.sharpe_ratio() - sharpe_flat_raw
        verdict = f"Compounding adds {ret_ratio:.1f}x to returns, Sharpe diff {sharpe_diff:.2f}."
    else:
        verdict = "(flat portfolio had zero/negative return)"
    print(f"\n  Verdict: {verdict}")
    ...
    try:
        ...
        fig.add_trace(
            go.Bar(x=rs.index, y=rs.values, name="Roll.Sharpe", marker_color=colors_rs),
            row=3,
            col=1,
        )
        fig.add_hline(
            y=1.0, line_dash="dash", line_color="white", annotation_text="Sharpe=1", row=3, col=1
        )
