...
Usage:
    ...
    weights = compute_erc_weights(
        strategy_returns={"gem_j5": gem_ret, "turtle": turtle_ret},
        target_vol_ann=0.10,  # optional rescale to target portfolio vol
    )
    # weights = {"gem_j5": 0.65, "turtle": 0.35} (illustrative)
"""
...
def _cov_matrix(strategy_returns: dict[str, pd.Series]) -> tuple[np.ndarray, list[str], int]:
    """Build covariance matrix from aligned strategy returns."""
...
def compute_erc_weights(
    ...
    *,
    ...
    """
    cov, names, n_obs = _cov_matrix(strategy_returns)
    n = len(names)
