...
NAMED_CRISES: list[tuple[str, date, date, str]] = [
    ("1987_blackmonday", date(1987, 10, 1), date(1987, 12, 31),
     "Oct 19 -22% one-day; Reagan crash"),
    ("2000_dotcom", date(2000, 3, 1), date(2002, 12, 31),
     "Dot-com bear, -49% SPY peak-trough"),
    ("2008_gfc", date(2008, 9, 1), date(2009, 3, 31),
     "Lehman + AIG; -57% peak-trough"),
    ("2010_flashcrash", date(2010, 5, 6), date(2010, 5, 7),
     "May 6 intraday -9%, recovered"),
    ("2011_eurozone", date(2011, 7, 1), date(2011, 10, 31),
     "US debt downgrade + EZ debt crisis"),
    ("2015_china", date(2015, 8, 1), date(2015, 10, 31),
     "Yuan devaluation, VIX spike"),
    ("2018_volmageddon", date(2018, 2, 1), date(2018, 2, 28),
     "Feb 5 XIV blowup"),
    ("2018_q4", date(2018, 10, 1), date(2018, 12, 31),
     "Fed-hike-cycle bear, -20% SPY"),
    ("2020_covid", date(2020, 2, 19), date(2020, 4, 30),
     "Mar -34% in 23 days"),
    ("2022_trend_reversal", date(2022, 1, 1), date(2022, 10, 31),
     "Inflation + Fed; bonds+stocks down"),
    ("2023_svb", date(2023, 3, 8), date(2023, 3, 31),
     "SVB collapse, regional bank stress"),
    ("2025_tradewar", date(2025, 3, 1), date(2025, 5, 31),
     "Tariff-driven equity selloff"),
]
