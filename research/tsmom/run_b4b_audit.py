"""B4b -- TSMOM audit on IBKR roll-stitched M1 data.

Specified by ``directives/Pre-Reg D2b B4b IBKR Roll-Stitched Futures
2026-05-15.md``. Sibling to ``run_b4_audit.py`` (yfinance M1).

Run via::

    uv run python research/tsmom/run_b4b_audit.py
...
REPORTS_DIR = PROJECT_ROOT / ".tmp" / "reports" / "b4b_tsmom_stitched"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

# Pre-reg B4b §2: shortened WFO due to ~3y stitched window (L41).
# B4 momentum lookback is typically 12 months -> we need IS >= 1.5y to
# leave any data for the rolling window.
...
# B4 also rebalances monthly; shorten momentum windows if needed.
# C3_window_24 would consume the entire IS; we keep it but expect
# under-data warnings.
...
def load_universe() -> pd.DataFrame:
    ...
    for root in UNIVERSE_ROOTS:
        try:
            ...
            continue
    if not parts:
        raise RuntimeError("No stitched M1 parquets found -- run stitch_root() first")
...
def _strategy_fn_for_cell(cfg: TsmomConfig, closes_visible):
...
def main():
    print("=" * 72)
    print("B4b -- TSMOM (MOP 2012) on IBKR Roll-Stitched M1")
    print("Pre-reg: directives/Pre-Reg D2b B4b IBKR Roll-Stitched Futures 2026-05-15.md")
    print("=" * 72)

    closes = load_universe()
    print(
        f"\nUniverse (stitched M1): {len(closes.columns)} commodities -- {list(closes.columns)}\n"
    ...
    if rel_spread > 0.30:
        ...
        with report.open("w", encoding="utf-8") as fh:
            fh.write("# B4b Audit -- ABORTED at plateau pre-flight\n\n")
    ...
    report = REPORTS_DIR / "result_log.md"
    with report.open("w", encoding="utf-8") as fh:
        fh.write("# B4b Audit Result Log -- TSMOM on IBKR Roll-Stitched M1\n\n")
        ...
        fh.write(
            f"- Gross (C8): Sharpe={c8.sharpe:+.4f}, CI_lo={c8.ci_lo:+.3f}\n"
            f"- Net   (C1): Sharpe={c1.sharpe:+.4f}, CI_lo={c1.ci_lo:+.3f}\n"
            f"- Cost drag: {c8.sharpe - c1.sharpe:+.4f}\n"
        )

        fh.write("\n## §4.4 B4b H1 falsification (stitched vs yfinance)\n\n")
        fh.write(
            "B4 (yfinance M1) canonical Sharpe was +0.16 (well below MOP's +1.0).\n"
            "If B4b stitched canonical Sharpe > +0.30, L40 was load-bearing — yfinance\n"
            "roll contamination was a meaningful drag. If B4b also < +0.30, L40 was NOT\n"
            "load-bearing; the universe-narrowness L39 is the dominant issue.\n\n"
        )
        if c1.sharpe > 0.30:
            fh.write(
                f"**H1 SUPPORTED** — C1 Sharpe {c1.sharpe:+.4f} > 0.30. L40 was load-bearing.\n"
            )
        else:
            fh.write(
                f"**H1 REJECTED** — C1 Sharpe {c1.sharpe:+.4f} <= 0.30. L40 was NOT "
                f"load-bearing for the magnitude gap. Universe narrowness L39 dominates.\n"
            )
        ...
        if deploy_eligible:
            ...
            fh.write(
                ...
                f"Build live class in titan/strategies/tsmom/.\n"
            )
        else:
            fh.write(
                "No cell deployment-eligible. Document negative result. "
                "TSMOM on the 24-commodity universe does not survive 5-axis "
