...
def _run_config(
    ...
    long_en, long_ex, short_en, short_ex, stop_pct = compute_signals(df, entry_p, exit_p)
...
def run_sweep(
    ...
    total = sum(1 for ep in entry_range for xp in exit_range if xp < ep)
...
def print_sweep_table(label: str, df_sweep: pd.DataFrame) -> None:
    ...
    print(
        f"  {'Entry':>6}  {'Exit':>5}  {'IS Sharpe':>10}  {'OOS Sharpe':>10}"
...
def print_top_configs(label: str, df_sweep: pd.DataFrame, n: int = TOP_N) -> None:
    ...
    for rank, (_, row) in enumerate(top.iterrows(), 1):
        ...
        print(f"\n  Rank #{rank}  —  ({ep}-bar entry / {xp}-bar exit)  Gate: {row['gate']}")
        print(THIN)
        print(f"  {'Metric':<26} {'IS':>12}  {'OOS':>12}")
        print(THIN)
        print(f"  {'Sharpe':<26} {_fmt(is_s['sharpe']):>12}  {_fmt(row['oos_sharpe']):>12}")
        print(
            f"  {'Total Return':<26} {_fmt(is_s['ret'], pct=True):>12}"
            f"  {_fmt(row['oos_ret'], pct=True):>12}"
        )
        print(
            f"  {'Max Drawdown':<26} {_fmt(is_s['dd'], pct=True):>12}"
            f"  {_fmt(row['oos_dd'], pct=True):>12}"
        )
        print(
            f"  {'Avg Drawdown':<26} {_fmt(is_s['avg_dd'], pct=True):>12}"
            f"  {_fmt(row['oos_avg_dd'], pct=True):>12}"
        )
        print(
            f"  {'Max DD Duration (bars)':<26} {is_s['max_dd_days']:>12}"
            f"  {int(row['oos_max_dd_days']):>12}"
        )
        print(
            f"  {'Win Rate':<26} {_fmt(is_s['wr'], pct=True):>12}"
            f"  {_fmt(row['oos_wr'], pct=True):>12}"
        )
        print(
            f"  {'Avg Win':<26} {_fmt(is_s['avg_win'], pct=True):>12}"
            f"  {_fmt(row['oos_avg_win'], pct=True):>12}"
        )
        print(
            f"  {'Avg Loss':<26} {_fmt(is_s['avg_loss'], pct=True):>12}"
        ...
        if ann:
            ...
            pairs = [
                (keys[i], keys[i + 1] if i + 1 < len(keys) else None)
...
def print_heatmap(label: str, df_sweep: pd.DataFrame, metric: str = "oos_sharpe") -> None:
    ...
    if df_sweep.empty:
        return
    pivot = df_sweep.pivot_table(index="entry_p", columns="exit_p", values=metric, aggfunc="mean")
    ...
    for ep in entries:
        row_vals = "".join(
            f"  {pivot.loc[ep, xp]:>+5.2f}" if xp in pivot.columns else "     -- " for xp in exits
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Turtle parameter sensitivity sweep")
    ...
    parser.add_argument("--start", default=None, help="Filter data from date, e.g. 2013-01-01")
    ...
    print(f"\n{SEP}")
    print(f"  TURTLE SWEEP  --  {args.instrument}  [{args.timeframe}]  direction={args.direction}")
    ...
    df_s1 = run_sweep(df, S1_ENTRY_RANGE, S1_EXIT_RANGE, args.timeframe, args.direction, s1_label)

    # System 2 sweep
    s2_label = f"{args.instrument} {args.timeframe} System-2 band"
    df_s2 = run_sweep(df, S2_ENTRY_RANGE, S2_EXIT_RANGE, args.timeframe, args.direction, s2_label)
