...
def print_leaderboard(rows: list[dict], system_num: int, entry_p: int, exit_p: int) -> None:
    ...
    if top_pass:
        ...
        for r in top_pass:
            ...
            vals = "".join(f"  {ann[y]:>+7.1%}" if y in ann else "       --" for y in years)
...
def print_pass_metrics(rows: list[dict], system_num: int, entry_p: int, exit_p: int) -> None:
    ...
    for r in ok:
        ...
        print(
            f"  {'Annualised Return':<{W}} {_fvp(is_s.get('ann_ret'))}  {_fvp(oos_s_raw.get('ann_ret') if oos_s_raw else float('nan'))}"
        )
        print(THIN)
        ...
        print(
            f"  {'Sortino':<{W}} {_fv(is_s.get('sortino')):>10}  {_fv(oos_s_raw.get('sortino') if oos_s_raw else float('nan')):>10}"
        )
        print(
            f"  {'Calmar':<{W}} {_fv(is_s.get('calmar')):>10}  {_fv(oos_s_raw.get('calmar') if oos_s_raw else float('nan')):>10}"
        )
        print(THIN)
        ...
        print(
            f"  {'Avg Drawdown':<{W}} {_fvp(is_s.get('avg_dd', float('nan')))}  {_fvp(oos_s_raw.get('avg_dd', float('nan')) if oos_s_raw else float('nan'))}"
        )
        ...
        print(
            f"  {'Payoff (|win/loss|)':<{W}} {_fv(payoff_is, '.2f'):>10}  {_fv(payoff_oos, '.2f'):>10}"
        )
        print(THIN)
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as pool:
        ...
        for future in as_completed(future_to_ticker):
            ...
            if result.get("error"):
                ...
                gate = result["gate"]
                print(
                    f"  [{completed:>3}/{len(tickers)}] {ticker:<12} OOS Sharpe {oos_sh:>+6.3f}  {gate}"
                )
            rows.append(result)
