...
Addresses model degradation. Financial regimes drift. A model trained in one market
environment will degrade as conditions change.

Provides:
  1. ModelHealthMonitor -- tracks regime dist, probability dist, and calibration drift.
  2. RetrainingScheduler -- defines when to retrain, manages warm-up on new model.
"""
...
class ModelHealthMonitor:
    ...
    Monitors a deployed model for three forms of degradation:
      1. Regime distribution drift (KL divergence from training distribution)
      2. Probability distribution drift (z-score of live mean vs training mean)
      3. Calibration drift (expected vs realised hit rate)
    """

    def __init__(
        self,
        training_regime_dist: np.ndarray,
        ...
        self._trade_outcomes: List[tuple] = []
    ...
    def record_trade(self, prob_success: float, outcome: int):
        """Record completed trade. outcome: 1 if upper barrier hit, 0 otherwise."""
...
class RetrainingScheduler:
    """
    Defines when retraining should occur and manages model warm-up.

    Triggers:
      1. Scheduled: every N bars (approximately annually on daily data)
      2. Alert-triggered: when ModelHealthMonitor fires an alert
    """

    def __init__(
        self,
        scheduled_interval_bars: int = 252,
        min_bars_for_trigger: int = 100,
        feature_warmup_bars: int = 150,
        hmm_warmup_bars: int = 60,
    ):
