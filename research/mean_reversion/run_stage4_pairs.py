...
def main() -> None:
    ...
    print(
        f"\n  Common bars: {len(df_eur):,}  [{df_eur.index[0].date()} -> {df_eur.index[-1].date()}]"
    )
    ...
    for split_name, sl in [("IS", slice(None, is_end)), ("OOS", slice(is_end, None))]:
        ...
        print(
            f"  {split_name}:  Sharpe={metrics['sharpe']:+.2f}  "
    ...
    for i in range(6, len(dates) - 2):
        ...
        m = pt.run_pairs_backtest(
            spread_oos_f, short_e, short_ex, long_e, long_ex, cost_oos_f, label=f"wfo_{i}"
        )
        fold_results.append(
            {
                "fold": i,
