...
def compute_hurst_h1_ffill(df_m5: pd.DataFrame) -> pd.Series:
    ...
    print(
        f"  Hurst range: [{hurst_m5.min():.3f}, {hurst_m5.max():.3f}]  mean={hurst_m5.mean():.3f}"
    )
    return hurst_m5
...
def main() -> None:
    ...
    print(
        f"  OOS: {len(df) - is_end:,} bars  [{df.iloc[is_end].name.date()} -> {df.index[-1].date()}]"
    )
    ...
    print(f"  P(ranging) mean={hmm_posterior.mean():.3f}  % bars >= 0.65: {pct_active:.1%}")
    ...
    sess_mask = (
        sig.session_mask(df.index, sess_parts)
    ...
    print(
        f"  Baseline (session only): Sharpe={baseline['sharpe']:+.2f}  "
    ...
    sweep_rows = Parallel(n_jobs=-1, prefer="threads")(delayed(_sweep_one)(p, h) for p, h in combos)
    ...
    print(
        sweep_df.head(5)[
            ["label", "p_thresh", "hurst_thresh", "sharpe", "sharpe_lift", "n_trades", "gate_pct"]
    ...
    fig.add_trace(
        go.Scatter(x=df_is.index, y=close_is, name="Close", line=dict(width=1)), row=1, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=hurst_is.index, y=hurst_is, name="Hurst", line=dict(color="purple", width=1)),
        row=3,
        col=1,
    )
