...
def load_h1(instrument: str) -> pd.DataFrame:
    ...
    print(f"  Loaded {instrument}_{TF}: {len(df):,} bars  "
...
def main() -> None:
    ...
    print(f"  Spread: ${SPY_SPREAD_DOLLARS:.3f} round-trip (~{SPY_SPREAD_DOLLARS / df['close'].mean() * 10000:.1f} bps)")
    ...
    report = {
        ...
        "gate": {"type": "atr_percentile", "atr_period": ATR_PERIOD,
    ...
    fig.update_layout(
        title=f"SPY H1 MR [ATR gate, us_open] -- {'PASSED' if all_passed else 'FAILED'}",
        height=700)
