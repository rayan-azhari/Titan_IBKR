...
def load() -> pd.DataFrame:
    ...
    df = df["2024-01-01":]
    print(
        f"  Loaded EUR/USD M5 (2024-2026): {len(df):,} bars  "
...
def plot_hourly(df: pd.DataFrame) -> None:
    ...
    fig.add_trace(
        go.Bar(x=grp["hour"], y=grp["volume"] / mean_vol, name="Rel Volume", marker_color="purple"),
        row=3,
        col=1,
    )
    ...
    if not sig.empty:
        print(
            f"     Significant drift hours (p<0.05): "
...
def plot_dow(df: pd.DataFrame) -> None:
    ...
    for d in dow_daily["dow"]:
        ...
        if len(vals) < 10:
            ...
            pvals.append(p)
    dow_daily["p"] = pvals
    dow_daily["sig"] = dow_daily["p"].apply(
        lambda p: "***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else ""
    )
    ...
    fig.add_trace(
        go.Bar(
            x=dow_daily["dow_name"],
            y=dow_daily["mean_day_ret"] * 100,
            text=labels,
            textposition="outside",
            error_y=dict(
                type="data",
    ...
    fig.update_layout(
        title="EUR/USD M5 (2024-2026) — Day-of-Week Profile", height=600, showlegend=False
    )
    path = OUT_DIR / "02_dow_profile.html"
    fig.write_html(str(path))
    print(f"  -> {path.name}")
    print(
        f"     DOW daily returns: {dict(zip(dow_daily['dow_name'], (dow_daily['mean_day_ret'] * 10000).round(1).tolist()))} (pips)"
    )
...
def plot_seasonal(df: pd.DataFrame) -> None:
    ...
    fig.add_trace(
        go.Bar(
            x=monthly["name"],
            y=monthly["mean_ret"] * 100,
            text=labels,
            textposition="outside",
            error_y=dict(
                type="data", array=(monthly["std_ret"] / np.sqrt(monthly["n"]) * 100).tolist()
            ),
            marker_color=colours,
            name="Mean Daily Ret",
        )
    ...
    for _, row in monthly.iterrows():
        fig.add_annotation(
            x=row["name"], y=-0.015, text=f"n={int(row['n'])}", showarrow=False, font=dict(size=9)
        )
    ...
    if not sig_months.empty:
        print(
            f"     Significant months: {list(zip(sig_months['name'], sig_months['mean_ret'].mul(10000).round(1)))}"
        )
...
def plot_am_pm_correlation(df: pd.DataFrame) -> None:
    ...
    pm = (
        df_wd[(df_wd["hour"] >= 12) & (df_wd["hour"] < 22)]
...
def plot_monday_gap(df: pd.DataFrame) -> None:
    ...
    print(
        f"     Monday gap stats: mean={gap_df['gap'].mean():.1f} pips  "
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=[
            "Monday Open Gap Distribution (pips)",
            "Gap vs Monday Return: does gap close?",
        ],
    )
    fig.add_trace(
        go.Histogram(x=gap_df["gap"], nbinsx=40, name="Gap (pips)", marker_color="steelblue"),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            ...
            y=gap_df["mon_return"],
            mode="markers",
            name="Gap vs Monday Return",
            marker=dict(color=gap_df["gap_closed"], colorscale=["red", "green"], size=6),
        ),
        row=1,
        col=2,
    )
    ...
    fig.update_layout(
        title="EUR/USD M5 (2024-2026) — Monday Gap Analysis", height=450, showlegend=False
    )
...
def plot_session_transition(df: pd.DataFrame) -> None:
    ...
    print(
        f"     NY open (13-17h) when London up   (n={len(lon_up)}):   "
        f"mean={lon_up.mean() * 10000:.1f} pips  p={p_up:.3f}"
    )
    print(
        f"     NY open (13-17h) when London down (n={len(lon_down)}): "
    ...
    fig.add_trace(
        go.Histogram(
            x=lon_up * 10000, name="London Up -> NY", opacity=0.7, marker_color="green", nbinsx=40
        ),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Histogram(
            x=lon_down * 10000, name="London Down -> NY", opacity=0.7, marker_color="red", nbinsx=40
        ),
        row=1,
        col=1,
    )
...
def plot_vol_clustering(df: pd.DataFrame) -> None:
    ...
    fig.add_hline(
        y=-1.96 / np.sqrt(len(hourly_tr)), line_dash="dot", line_color="orange", row=1, col=1
    )
    ...
    fig.update_layout(
        title="EUR/USD M5 (2024-2026) — Volatility Autocorrelation", height=600, showlegend=False
    )
