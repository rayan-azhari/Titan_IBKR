...
def plot_hourly(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig.add_trace(go.Bar(x=grp["hour"], y=grp["abs_ret"] * 100,
    ...
    fig.add_trace(go.Bar(x=grp["hour"], y=grp["drift"] * 100,
                         text=labels, textposition="outside",
                         marker_color=colours), row=2, col=1)
    ...
    fig.update_xaxes(tickvals=list(range(8, 23)),
                     ticktext=[f"{h:02d}h" for h in range(8, 23)])
    ...
    sig = grp[grp["p"] < 0.05]
    return {
        "sig_drift_hours": [(int(r.hour), round(r.drift * 100, 4), r.sig) for _, r in sig.iterrows()],
    }
...
def plot_open_close_hours(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    for col, hour, label in [(1, first_hour, "Open"), (2, last_hour, "Close")]:
        ...
        fig.add_trace(go.Histogram(x=vals, nbinsx=50, name=label, marker_color=colour,
                                   opacity=0.7), row=1, col=col)
        fig.add_vline(x=vals.mean(), line_dash="dash", line_color="black", row=1, col=col,
...
def plot_dow(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig.add_trace(go.Bar(x=vol_grp["name"], y=vol_grp["abs_ret"] * 100,
                         marker_color="steelblue"), row=1, col=1)

    colours = ["red" if d < 0 else "green" for d in dow_d["mean"]]
    labels  = [f"{d*100:.3f}%{s}" for d, s in zip(dow_d["mean"], dow_d["sig"])]
    fig.add_trace(go.Bar(x=dow_d["name"], y=dow_d["mean"] * 100,
                         text=labels, textposition="outside",
                         error_y=dict(type="data",
                                      array=(dow_d["std"] / np.sqrt(dow_d["n"]) * 100).tolist()),
                         marker_color=colours), row=2, col=1)
...
def plot_intraday_path_by_dow(df: pd.DataFrame, symbol: str, out: Path) -> None:
    ...
    for dow in range(5):
        ...
        fig.add_trace(go.Scatter(x=avg.index, y=avg.values,
                                 name=DOW_NAMES[dow], mode="lines+markers"))

    fig.add_vline(x=SESSION_OPEN_UTC, line_dash="dot", line_color="grey",
                  annotation_text="open")
    fig.add_vline(x=SESSION_CLOSE_UTC, line_dash="dot", line_color="grey",
                  annotation_text="close")
    fig.update_layout(
        title=f"{symbol} H1 — Mean Normalised Intraday Path by DOW",
        xaxis_title="UTC Hour", yaxis_title="Normalised cumulative return",
        height=450,
    )
...
def plot_monthly(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig = go.Figure(go.Bar(
        x=monthly["name"], y=monthly["mean"] * 100,
        text=labels, textposition="outside",
        error_y=dict(type="data",
...
def plot_am_pm(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig = px.scatter(both, x="early", y="late", color="dow_name", opacity=0.5,
                     labels={"early": f"Early session (<{mid_h}h UTC)",
...
def plot_vol_autocorr(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig.update_layout(
        ...
        xaxis_title="Lag (H1 bars)", yaxis_title="ACF", height=380, showlegend=False,
    )
    ...
    for dow in pivot.index:
        for hour in pivot.columns:
            ...
            if len(vals) >= 30:
                ...
                if p < 0.05:
                    sig_cells.append((DOW_NAMES[dow], int(hour),
                                      round(float(pivot.loc[dow, hour]), 4),
                                      round(float(p), 4), marker))
...
def plot_yearly_drift(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    yr_grp = yearly.groupby("year").agg(
        mean=("day_ret", "mean"), std=("day_ret", "std"), n=("day_ret", "count"),
    ).reset_index()

    colours = ["red" if d < 0 else "steelblue" for d in yr_grp["mean"]]
    fig = go.Figure(go.Bar(
        x=yr_grp["year"].astype(str), y=yr_grp["mean"] * 100,
        error_y=dict(type="data",
                     array=(yr_grp["std"] / np.sqrt(yr_grp["n"]) * 100).tolist()),
        marker_color=colours,
    ...
    return {"pos_years": pos_years, "neg_years": neg_years,
...
def run_instrument(symbol: str) -> dict | None:
    ...
    print(f"\n{'='*55}")
    print(f"  {symbol}  ({is_type}, {n_days} days, "
    ...
    results: dict = {
        "symbol": symbol, "skipped": False, "n_days": n_days,
        "type": is_type,
    ...
    if results.get("open_hour_sig"):
        print(f"  ** Open hour:        mean={results.get('open_hour_mean_pct')}%  "
              f"p={results.get('open_hour_p')}  {results.get('open_hour_sig')}")
    if results.get("close_hour_sig"):
        print(f"  ** Close hour:       mean={results.get('close_hour_mean_pct')}%  "
              f"p={results.get('close_hour_p')}  {results.get('close_hour_sig')}")
    print(f"  ** Early/late r:     {results.get('early_late_r')}  "
          f"p={results.get('early_late_p')}")
    ...
    if results.get("sig_clv_cells"):
        for dow_name, hour, mean_clv, p, marker in results["sig_clv_cells"][:6]:
            ...
            print(f"  ** CLV{sig_str} {dow_name} {hour:02d}h: {direction}{abs(mean_clv):.4f}  (p={p:.4f})")
...
def build_summary(all_results: list[dict]) -> None:
    ...
    fig = make_subplots(rows=3, cols=1, vertical_spacing=0.12, subplot_titles=[
    ...
    fig.update_layout(title="H1 Pattern Analysis — Cross-Instrument Summary",
    ...
    # Text table
    print("\n" + "=" * 110)
    print(f"  {'Symbol':<14} {'Days':>5} {'VolACF':>7} {'E/L r':>7} {'E/L p':>7} "
    ...
    for r in sorted(valid, key=lambda x: x.get("tr_lag1_acf", 0), reverse=True):
        ...
        print(f"  {r['symbol']:<14} {r['n_days']:>5} "
