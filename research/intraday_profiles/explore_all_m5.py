...
def plot_hourly(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig.add_trace(
        go.Bar(x=grp["hour"], y=grp["abs_ret"] * 10000, marker_color="steelblue", name="Vol"),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(
            x=grp["hour"], y=grp["volume"] / max(mv, 1e-9), marker_color="purple", name="RelVol"
        ),
        row=3,
        col=1,
    )
    ...
    return {
        "sig_drift_hours": [
            (int(r.hour), round(r.drift * 10000, 2), r.sig) for _, r in sig.iterrows()
        ],
    }
...
def plot_dow(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig.add_trace(
        go.Bar(
            x=vol_grp["name"], y=vol_grp["abs_ret"] * 10000, marker_color="steelblue", name="Vol"
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            text=labels,
            textposition="outside",
            error_y=dict(type="data", array=(dow_d["std"] / np.sqrt(dow_d["n"]) * 10000).tolist()),
            marker_color=colours,
            name="DayRet",
        ),
        row=2,
...
def plot_intraday_path_by_dow(df: pd.DataFrame, symbol: str, out: Path) -> None:
    ...
    for dow in range(5):
        ...
        fig.add_trace(go.Scatter(x=avg.index, y=avg.values, name=DOW_NAMES[dow], mode="lines"))
    ...
    fig.update_layout(
        title=f"{symbol} — Mean Normalised Intraday Path by DOW",
...
def plot_monthly(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    for _, row in monthly.iterrows():
        fig.add_annotation(
            x=row["name"],
            y=monthly["mean"].min() * 10000 - 2,
            text=f"n={int(row['n'])}",
            showarrow=False,
            font=dict(size=9),
        )
    fig.update_layout(
        title=f"{symbol} — Mean Daily Return by Month  * = p<0.05",
        yaxis_title="Pips",
        height=430,
        showlegend=False,
    )
    ...
    return {
        "sig_months": [
            (r["name"], round(r["mean"] * 10000, 1), r["sig"]) for _, r in sig.iterrows()
        ]
    }
...
def plot_am_pm(df: pd.DataFrame, symbol: str, out: Path, is_fx: bool) -> dict:
    ...
    pm = (
        df_wd[(df_wd["hour"] >= pm_start) & (df_wd["hour"] < pm_end)]
    ...
    fig = px.scatter(
        ...
        y="pm",
        labels={
            "am": f"AM return (00-{am_end}h UTC)",
            "pm": f"PM return ({pm_start}-{pm_end}h UTC)",
        },
    ...
    return {
        ...
        "pm_when_am_down_pips": round(float(am_down.mean()) * 10000, 1)
        if len(am_down)
        else float("nan"),
    }
...
def plot_session_transition(df: pd.DataFrame, symbol: str, out: Path, is_fx: bool) -> dict:
    ...
    df_wd = df[df["dow"] < 5]
    london = (
        df_wd[(df_wd["hour"] >= 7) & (df_wd["hour"] < 12)]
        .groupby("date")["log_ret"]
        .sum()
        .rename("london")
    )
    ny = (
        df_wd[(df_wd["hour"] >= 13) & (df_wd["hour"] < 17)]
        .groupby("date")["log_ret"]
        .sum()
        .rename("ny")
    )
    ...
    fig.add_trace(
        go.Histogram(
            x=lon_up * 10000, name="London Up", opacity=0.7, marker_color="green", nbinsx=40
        ),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Histogram(
            x=lon_down * 10000, name="London Down", opacity=0.7, marker_color="red", nbinsx=40
        ),
        row=1,
        col=1,
    )
    ...
    fig.update_layout(
        title=f"{symbol} — London -> NY Follow-Through  (r={r:.3f}, p={p:.3f})",
        height=430,
        barmode="overlay",
    )
    ...
    return {
        "london_ny_r": round(r, 3),
        "london_ny_p": round(p, 3),
...
def plot_vol_autocorr(df: pd.DataFrame, symbol: str, out: Path) -> dict:
    ...
    fig.add_trace(
        go.Bar(x=lags, y=h_acf, marker_color=["red" if a < 0 else "steelblue" for a in h_acf]),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(
            x=list(range(1, 7)),
            y=b_acf,
            marker_color=["red" if a < 0 else "steelblue" for a in b_acf],
        ),
        row=2,
        col=1,
    )
...
def build_summary(all_results: list[dict]) -> None:
    ...
    fig.add_trace(
        go.Bar(x=syms, y=acf_vals, marker_color="steelblue", name="Vol ACF lag-1h"), row=1, col=1
    )
    ...
    fig.add_trace(go.Bar(x=syms, y=am_pm_r, marker_color=colours, name="AM/PM r"), row=2, col=1)
    fig.add_hline(y=0, line_dash="dot", row=2, col=1)

    lon_r = [r.get("london_ny_r", float("nan")) for r in valid]
    fig.add_trace(go.Bar(x=syms, y=lon_r, marker_color="orange", name="London->NY r"), row=3, col=1)
    fig.add_hline(y=0, line_dash="dot", row=3, col=1)

    fig.update_layout(
        title="M5 Pattern Analysis — Cross-Instrument Summary", height=750, showlegend=False
    )
    ...
    print("\n" + "=" * 75)
    print(
        f"  {'Symbol':<18} {'Days':>5} {'Vol ACF':>8} {'AM/PM r':>8} {'Lon->NY r':>10}  Sig patterns"
    )
    print("  " + "-" * 73)
    for r in valid:
        sig_parts = []
