"""run_signal_combination.py -- Signal Combination IC/ICIR Analysis.
...
IC/ICIR than any individual signal at a given horizon. Six methods are tested:
    ...
    1. Correlation clustering  -- identify redundant signal pairs (diagnostic)
...
Look-ahead safety:
    All signal computations are causal (inherited from run_signal_sweep.py).
...
# ── Method 1: Correlation clustering ──────────────────────────────────────────
...
def method_correlation_analysis(
    ...
    Uses union-find to group connected components (avoids transitive false-positives
    from naive pairwise thresholding).
...
def method_pca(
    ...
    logger.info("PCA PC1 explains %.1f%% of variance across %d USABLE signals", ev * 100, clean.shape[1])
...
def _print_report(
    ...
    print(f"  Bars: {n_bars:,}  |  USABLE signals: {n_usable}  |  Threshold: |IC| >= {IC_USABLE_THRESHOLD:.3f}")
    print("=" * W)
...
def run_combination(
    ...
    # 8. Method 1: correlation clustering
    logger.info("Method 1: correlation clustering...")
    ...
    for r in combo_results:
        ...
        rows.append({
            ...
            "vs_best_ind": delta,
            "signals_used": ",".join(r["signals_used"]),
        })
    result_df = pd.DataFrame(rows)
    out_path = report_dir / f"ic_combo_{slug}_h{horizon}.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Signal Combination IC/ICIR Analysis")
