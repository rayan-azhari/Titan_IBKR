...
def _trade_markers(pf, direction: str) -> tuple[list, list, list]:
    ...
    for _, row in trades.iterrows():
        entry_col = next(
            (c for c in row.index if "entry" in c.lower() and "idx" in c.lower()), None
        )
...
def build_chart(instrument: str, threshold: float) -> Path:
    ...
    # Threshold bands
    for sign, label in [
        (threshold, f"+{threshold}z (long)"),
        (-threshold, f"−{threshold}z (short)"),
    ]:
        fig.add_hline(
            y=sign,
            line_dash="dash",
            ...
            annotation_font_color=C_THRESH,
            annotation_font_size=10,
            row=2,
            col=1,
        )
    fig.add_hline(
        y=0, line_dash="dot", line_color=C_TEXT, line_width=0.5, opacity=0.3, row=2, col=1
    )
    ...
    fig.update_yaxes(
        type="log", tickformat=".0f", row=3, col=1, title_text="Portfolio value (log, start=100)"
    )
    ...
    if wfo_df is not None:
        ...
        fig.add_hline(
            y=0, line_dash="dot", line_color=C_TEXT, line_width=0.5, opacity=0.4, row=4, col=1
        )
        fig.add_hline(
            y=1, line_dash="dash", line_color=C_THRESH, line_width=0.8, opacity=0.6, row=4, col=1
        )
