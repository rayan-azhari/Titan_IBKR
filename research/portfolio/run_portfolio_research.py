...
def main() -> None:
    ...
    parser.add_argument(
        "--wfo",
        action="store_true",
        help="Run Walk-Forward Optimization instead of static portfolio analysis.",
    )
    parser.add_argument("--is-days", type=int, default=504, help="WFO: IS window (trading days)")
    parser.add_argument("--oos-days", type=int, default=126, help="WFO: OOS window (trading days)")
    parser.add_argument(
        "--wfo-type",
        default="rolling",
        choices=["rolling", "anchored"],
        help="WFO: rolling (fixed IS) or anchored (IS grows from day 0)",
    )
    ...
    # ── Default to current live portfolio ────────────────────────────────
    if args.strategies is None:
        ...
        print("  [INFO] No --strategies specified. Using current live portfolio.")

    # ── WFO mode: delegate to run_portfolio_wfo ──────────────────────────
    if args.wfo:
        from research.portfolio.run_portfolio_wfo import (
            main as wfo_main,
        )

        # Override sys.argv so argparse in wfo_main picks up our args
        sys.argv = [
            "run_portfolio_wfo.py",
            "--strategies",
            *args.strategies,
            "--weighting",
            args.weights[0] if args.weights else "risk_parity",
            "--is-days",
            str(args.is_days),
            "--oos-days",
            str(args.oos_days),
