...
def run_l65_joint(
    gem: pd.Series, turtle: pd.Series, i1v2: pd.Series,
...
def main() -> None:
    ...
    print(f"  turtle_cat: {len(turtle)} bars ({turtle.index[0].date()} -> {turtle.index[-1].date()})")
    i1v2 = i1v2_c6_returns().dropna()
    print(f"  i1v2_c6: {len(i1v2)} bars ({i1v2.index[0].date()} -> {i1v2.index[-1].date()})")
    print(f"  i1v2_c6 stats: mean={i1v2.mean():+.6f}, std={i1v2.std():.6f}, "
    ...
    for w, r in single.items():
        ...
        print(f"  w={w:.0%}: P_kill={r['p_kill']*100:.3f}%, "
    ...
    for key, r in joint.items():
        w = r["weights"]
        verdict = "PASS" if r["passes"] else "FAIL"
        print(f"  {w}: P_kill={r['p_kill']*100:.3f}%, "
    ...
    with report_fp.open("w", encoding="utf-8") as fh:
        ...
        fh.write("**Pre-reg:** `directives/Pre-Reg I1v2 Multi-Feature HMM Regime Gate 2026-05-17.md`\n")
        fh.write("**Audit verdict:** C6_smoothed DEPLOY (Sharpe +0.52, CI_lo +0.049, noise=best)\n\n")
        fh.write("## Strategy return series\n\n")
        fh.write(f"- gem_j5: {len(gem)} bars\n")
        fh.write(f"- turtle_cat: {len(turtle)} bars\n")
        fh.write(f"- i1v2_c6: {len(i1v2)} bars, ann Sharpe = {i1v2.mean()/i1v2.std()*((252)**0.5):+.4f}\n\n")
        fh.write("## L65 single-strategy ruin (I1v2 C6 standalone)\n\n")
        fh.write("| Weight | P_kill | 95th-pct DD | 50th-pct DD | Verdict |\n")
        fh.write("|---:|---:|---:|---:|:---:|\n")
        for w, r in single.items():
            fh.write(f"| {w:.0%} | {r['p_kill']*100:.3f}% | {r['p95_maxdd']*100:.2f}% | "
        ...
        for key, r in joint.items():
            w = r["weights"]
            fh.write(f"| {w['gem']:.0%} | {w['turtle']:.0%} | {w['i1v2']:.0%} | "
