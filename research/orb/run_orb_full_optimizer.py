...
SP100_TICKERS = [
    ...
    "JPM", "XOM", "UNH", "WMT", "MA", "JNJ", "PG", "HD", "ORCL", "COST", 
    "ABBV", "CVX", "CRM", "BAC", "MRK", "KO", "NFLX", "AMD", "PEP", "LIN", 
    "TMO", "ADBE", "WFC", "CSCO", "MCD", "ABT", "TMUS", "INTU", "IBM", "GE", 
    "CAT", "QCOM", "TXN", "AMAT", "DHR", "VZ", "PFE", "PM", "ISRG", "NOW", 
    "COP", "BA", "SPGI", "HON", "AMGN", "RTX", "UNP", "LOW", "INTC", "SYK", 
    "GS", "NEE", "ELV", "BLK", "TJX", "PGR", "AXP", "MDT", "C", "LMT", 
    "UBER", "VRTX", "CB", "REGN", "MMC", "ADI", "BSX", "CI", "CVS", "ZTS", 
    "T", "FI", "SLB", "MDLZ", "MO", "BKNG", "GILD", "EOG", "BDX", "SO", 
    "CME", "NOC", "CSX", "ITW", "DUK", "CL", "ICE", "FDX", "USB", "PNC"
]
...
def evaluate_ticker(ticker: str, days: int = 59) -> list[dict]:
    ...
    df_1d = yf.download(ticker, start=start_date - timedelta(days=100), end=end_date, interval="1d", progress=False)
    ...
    rsi_filters = [True, False]   # True = Requires RSI to NOT be overextended (<70 for long, >30 for short)
    ...
    for atr_m, use_sma, use_rsi in itertools.product(atr_mults, sma_filters, rsi_filters):
        ...
        pf = vbt.Portfolio.from_signals(
            df_5m["Close"], entries=entries, exits=exits,
        ...
        if trades > 0:
            ...
            if returns > 0 and sharpe > 0: # Only store strictly profitable configurations to save memory
                results.append({
                    "ticker": ticker,
                    "atr": atr_m,
                    "sma": use_sma,
                    ...
                    "trades": trades,
                    "profit_factor": pf.trades.profit_factor() if not np.isnan(pf.trades.profit_factor()) else 0
                })
    return results

if __name__ == "__main__":
    print(f"Starting Multi-Threaded Grid Search Optimization on Top 100 Tickers...")
    ...
    print("==========================================================================================")
    print(f"{'Ticker':<8} | {'Return':<8} | {'Win Rate':<10} | {'Trades':<8} | {'ATR Stop':<10} | {'SMA Filter':<12} | {'RSI Filter'}")
    print("-" * 90)
    ...
    for r in flat_results[:25]:
        ...
        print(f"{r['ticker']:<8} | {ret_str:<8} | {win_str:<10} | {r['trades']:<8} | {r['atr']:<10.1f} | {sma_str:<12} | {rsi_str}")
    print("==========================================================================================")
