...
def fetch_sp100_tickers() -> list[str]:
    ...
    return [
        ...
        "BRK-B", "LLY", "AVGO", "V", "JPM", "XOM", "UNH", "WMT", 
        "MA", "JNJ", "PG", "HD", "ORCL"
    ]
...
def run_advanced_orb(ticker: str, df_spy: pd.DataFrame, days: int = 59) -> dict | None:
    ...
    df_1d = yf.download(ticker, start=start_date - timedelta(days=20), end=end_date, interval="1d", progress=False)
    ...
    if isinstance(df_spy_1d.columns, pd.MultiIndex): df_spy_1d.columns = df_spy_1d.columns.droplevel(1)
    ...
    df_5m["RS"] = df_5m["Date_Str"].map(df_1d.set_index("Date_Str")["RS"].shift(1).to_dict()) # Use yesterday's RS
...
if __name__ == "__main__":
    ...
    end_date = datetime.now()
    start_date = end_date - timedelta(days=days - 1)
    df_spy = yf.download("SPY", start=start_date - timedelta(days=20), end=end_date, interval="1d", progress=False)
    ...
    print("="*80)
    print(f"{'Ticker':<8} | {'Return':<8} | {'Win Rate':<10} | {'Trades':<8} | {'Profit Factor':<12} | {'Sharpe'}")
    print("-" * 80)
    ...
    for r in results:
        ...
        print(f"{r['ticker']:<8} | {ret_str:<8} | {win_str:<10} | {r['trades']:<8} | {r['profit_factor']:<12.2f} | {r['sharpe']:.2f}")
    print("="*80)
