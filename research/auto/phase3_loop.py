...
experiments = [
    ...
    (
        "IWB lstm_e2e hidden=64",
        c(ML_BASE, strategy="lstm_e2e", instruments=["IWB"], lstm_hidden=64),
    ),
    ...
    ("MR AUD_JPY spread1", c(MR_BASE, instruments=["AUD_JPY"], spread_bps=1.0, slippage_bps=0.5)),
    (
        "MR AUD_JPY D",
        c(MR_BASE, instruments=["AUD_JPY"], timeframe="D", is_bars=1500, oos_bars=375),
    ),
    # ── 7. Cross-asset deeper ─────────────────────────────────────────────────
    # IEF->GLD lb40 scored 1.46 (best XA so far). Try fine-tuning.
    (
        "XA IEF->GLD lb40 hd20",
        c(XA_BASE, instruments=["GLD"], bond="IEF", lookback=40, hold_days=20),
    ),
    (
        "XA IEF->GLD lb30 hd15",
        c(XA_BASE, instruments=["GLD"], bond="IEF", lookback=30, hold_days=15),
    ),
    (
        "XA IEF->GLD lb20 thr0",
        c(XA_BASE, instruments=["GLD"], bond="IEF", lookback=20, hold_days=20, threshold=0.0),
    ),
    ("XA HYG->GLD lb10", c(XA_BASE, instruments=["GLD"], bond="HYG")),
    ("XA LQD->QQQ lb10", c(XA_BASE, instruments=["QQQ"], bond="LQD")),
    ("XA IEF->DBC lb20", c(XA_BASE, instruments=["DBC"], bond="IEF", lookback=20, hold_days=20)),
