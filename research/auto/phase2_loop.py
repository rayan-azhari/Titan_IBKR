...
def git_commit(msg):
    ...
    r = subprocess.run(["git", "commit", "-m", f"exp: {msg}"], cwd=ROOT,
...
experiments = [
    ...
    ("IWB stacking xgb depth=5", c(ML_BASE, instruments=["IWB"],
                                   xgb_params=dict(n_estimators=300, max_depth=5, learning_rate=0.03,
                                                   subsample=0.8, colsample_bytree=0.6,
                                                   random_state=42, verbosity=0))),
    ("IWB stacking xgb depth=3", c(ML_BASE, instruments=["IWB"],
                                   xgb_params=dict(n_estimators=300, max_depth=3, learning_rate=0.03,
                                                   subsample=0.8, colsample_bytree=0.6,
                                                   random_state=42, verbosity=0))),
    ("IWB stacking xgb n500", c(ML_BASE, instruments=["IWB"],
    ...
    ("IWB stacking 4labels", c(ML_BASE, instruments=["IWB"],
                               label_params=[
                                   dict(rsi_oversold=45, rsi_overbought=55, confirm_bars=5, confirm_pct=0.005),
                                   dict(rsi_oversold=50, rsi_overbought=50, confirm_bars=5, confirm_pct=0.003),
                                   dict(rsi_oversold=48, rsi_overbought=52, confirm_bars=5, confirm_pct=0.005),
    ...
    ("TCN stacking IWB lookback=30", c(ML_BASE, strategy="tcn_stacking", instruments=["IWB"], lookback=30)),
    ("TCN stacking IWB lookback=10", c(ML_BASE, strategy="tcn_stacking", instruments=["IWB"], lookback=10)),
    ("TCN stacking QQQ", c(ML_BASE, strategy="tcn_stacking", instruments=["QQQ"])),

    # ── 4. AE stacking on IWB ─────────────────────────────────────────────────
    ("AE stacking IWB k4", c(ML_BASE, strategy="ae_stacking", instruments=["IWB"],
                             ae_latent_dim=8, ae_clusters=4, ae_epochs=100)),
    ("AE stacking IWB k3", c(ML_BASE, strategy="ae_stacking", instruments=["IWB"],
                             ae_latent_dim=8, ae_clusters=3, ae_epochs=100)),
    ("AE stacking IWB k6", c(ML_BASE, strategy="ae_stacking", instruments=["IWB"],
                             ae_latent_dim=8, ae_clusters=6, ae_epochs=100)),
    ("AE stacking IWB latent16", c(ML_BASE, strategy="ae_stacking", instruments=["IWB"],
                                   ae_latent_dim=16, ae_clusters=4, ae_epochs=100)),
    ...
    ("MR AUD_JPY donchian", c(MR_BASE, instruments=["AUD_JPY"],
    ...
    ("XA IEF->GLD lb40", c(XA_BASE, instruments=["GLD"], bond="IEF",
    ...
    ("XA IEF->HYG lb20", c(XA_BASE, instruments=["HYG"], bond="IEF",
