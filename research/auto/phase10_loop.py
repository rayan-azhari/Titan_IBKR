...
experiments = [
    ...
    ("MR AUD_JPY v56 sp0.25", c(MR_V46_SP0, vwap_anchor=56,
                                spread_bps=0.25, slippage_bps=0.1)),
    ("MR AUD_JPY v56 sp0", c(MR_V46_SP0, vwap_anchor=56)),

    # ── 2. Verify v46 sp0.5 with slight is/oos variation ─────────────────────
    ("MR AUD_JPY v46 sp0.5 is28k oos8k", c(MR_V46_REAL,
                                            is_bars=28000, oos_bars=8000)),
    ("MR AUD_JPY v46 sp0.5 is32k oos8k", c(MR_V46_REAL,
    ...
    ("Portfolio IWB40+AJ40+HYG20", {
        ...
        "strategies": [
            ...
            dict(strategy="cross_asset", instruments=["IWB"], bond="HYG",
                 lookback=10, hold_days=10, threshold=0.50,
                 is_days=504, oos_days=126, spread_bps=5.0, weight=0.2),
        ],
    }),

    # ── 4. H4 timeframe for AUD/JPY (novel: 4-hour bars) ─────────────────────
    ("MR AUD_JPY H4 v12 don", dict(
        strategy="mean_reversion", instruments=["AUD_JPY"], timeframe="H4",
        vwap_anchor=12, regime_filter="conf_donchian_pos_20",
        tier_grid="conservative", spread_bps=0.5, slippage_bps=0.2,
        is_bars=7500, oos_bars=1875)),
    ("MR AUD_JPY H4 v6 don", dict(
        strategy="mean_reversion", instruments=["AUD_JPY"], timeframe="H4",
        vwap_anchor=6, regime_filter="conf_donchian_pos_20",
        tier_grid="conservative", spread_bps=0.5, slippage_bps=0.2,
        is_bars=7500, oos_bars=1875)),
