...
def trade_stats(trades: list[float]) -> dict:
    ...
    if not trades:
        return dict(n_trades=0, win_rate=0.0, n_wins=0, n_losses=0,
                    avg_win=0.0, avg_loss=0.0, profit_factor=0.0,
                    best_trade=0.0, worst_trade=0.0, avg_trade=0.0)
...
def risk_of_ruin(trades: list[float], n_sims: int = 5000,
...
def get_raw_results(strat: dict) -> dict | None:
    ...
    print(f"OK  sharpe={r.get('sharpe','?'):.3f}  "
...
def report_strategy(name: str, ret: pd.Series, trades: list[float],
                    weight: float, stop_mult: float) -> None:
    print(f"\n{SEP}")
    ...
    print()
    print(f"  --- Return Metrics (1% ATR risk-scaled) ---")
    ...
    if ann:
        print(f"  --- Annual Returns (risk-scaled) ---")
...
def report_portfolio(port_ret: pd.Series, strategy_names: list[str],
                     clipped: list[pd.Series], weights: list[float]) -> float:
    ...
    print()
    print(f"  --- Trade Statistics (portfolio level) ---")
    print(f"  Total Trades    : {ts['n_trades']}")
    print(f"  Win Rate        : {ts['win_rate']:.1f}%  ({ts['n_wins']} W / {ts['n_losses']} L)")
    print(f"  Avg Win         : +{ts['avg_win']:.2f}%")
    print(f"  Avg Loss        : {ts['avg_loss']:.2f}%")
    ...
    print(f"  Max simultaneous loss (all positions hit stop): {max_concurrent_loss*100:.2f}% of total capital")
    print()
...
def main():
    ...
    scaled_rets = [scale_to_risk(ret, s["stop_mult"])
                   for ret, s in zip(raw_rets, valid_strats)]

    # ── Step 3: Extract per-trade data ─────────────────────────────────────
    all_trades = []
    for r, strat, runner_type in zip(raw_results, valid_strats,
                                     [s["runner"] for s in valid_strats]):
        if runner_type == "mr":
    ...
    strategy_names = [s["name"] for s in valid_strats]
    weights_list = [s["target_weight"] for s in valid_strats]

    for i, (strat, ret, trades) in enumerate(zip(valid_strats, raw_rets, all_trades)):
        report_strategy(strat["name"], ret, trades,
                        strat["target_weight"], strat["stop_mult"])
    ...
    for i in skipped:
        print(f"  EXCLUDED (no overlap): {valid_strats[i]['name']} "
    ...
    print(f"  Composite SCORE         : {score:.4f}")
    print(f"  Baseline (AUD/JPY solo) : 5.1368")
    print(SEP)
